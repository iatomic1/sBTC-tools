...
export async function calculateTransferStats(): Promise<{
  ...
  try {
    const transactions = await fetchAllRecentTransactions(30);
    ...
    const amounts = transferTxs.map((tx) => {
      ...
      const amountArg = args.find((arg) => arg.type === 'uint');
      if (amountArg && amountArg.type === 'uint') {
        const amountStr = amountArg.repr.startsWith('u')
          ? amountArg.repr.substring(1)
          : amountArg.repr;
        return Number.parseFloat(amountStr) / 100000000; // Convert from satoshis to sBTC (8 decimal places)
      }
      return 0;
    });
    ...
    const largestTx = maxIndex >= 0 ? transferTxs[maxIndex] : undefined;
    const largestTransferDate = largestTx
      ? new Date(largestTx.tx.block_time * 1000).toLocaleDateString('en-US', {
          month: 'short',
          day: 'numeric',
          year: 'numeric',
        })
      ...
      return txs.reduce((sum, tx) => {
        ...
        if (amountArg && amountArg.type === 'uint') {
          const amountStr = amountArg.repr.startsWith('u')
            ? amountArg.repr.substring(1)
            : amountArg.repr;
          return sum + Number.parseFloat(amountStr) / 100000000;
        }
        return sum;
      }, 0);
    };
